...
UNITS = 200 # Total number of units traded per ticker
RETAIN_UNITS = 20 # total units to retain after tp (don't sell all)
...
class Profit:  # Keeps Track of all the Trades pnl
    ...
    def fill_tp(cls, grid:GridLine):
        # Since this is only DCA bot profit can only be taken on buys hence grid.last_grid_line is passed in as parameter
        grid.in_trade = False
    ...
    def p(cls, _grid :GridLine):
        print(f"profit : {cls.profit}\tfee : {cls.fee}\tUnitsLeft : {cls.total_units_left}\ttotal trades : {cls.total_trades_executed}\tatGrid : {_grid.name}\tHoldingValue : {_grid.price * cls.total_units_left}")
...
def move_price(): # Price Tracker 
    ...
    _grid_start_price = 0.08544
    _decimals = 5

    # Get Price from csv for now
    price_movment = simulate_price_movment_in_markets()
    last_price = 0  # set this equal to market price

    # Create Grid Lines
    grid_lines = GridLineManager(_grid_start_price,TP, _decimals)
    grid_lines.calculate_grid_lines(
        _grid_start_price 
    )
    grid_lines.create_grid_line_objects()
